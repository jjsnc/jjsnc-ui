<template>
  <transition name="jjsnc-action-sheet-fade">
    <jjsnc-popup
      type="action-sheet"
      :class="{'jjsnc-action-sheet_picker':pickerStyle}"
      :center="false"
      :mask="true"
      :z-index="zIndex"
      v-show="isVisible"
      @mask-click="maskClick"
    >
      <transition name="jjsnc-action-sheet-move">
        <div class="jjsnc-action-sheet-panel jjsnc-safe-area-pb" v-show="isVisoble" @click.stop>
          <h1
            class="jjsnc-action-sheet-title border-tottom-1px"
            v-show="pickerStyle || title"
          >{{title}}</h1>
          <div class="jjsnc-action-sheet-centent">
            <ul class="jjsnc-action-sheet-list">
              <li
                class="jjsnc-action-sheet-item border-bottom-1px"
                v-for="(item, index) in data"
                :key="index"
                :data-align="item.align"
                :class="[
                 item.class || '',
                 index===active? 'jjsnc-action-sheet-item_active': ''
                 ]"
                v-html="item.content"
                @click="itemClick(item, index)"
              ></li>
            </ul>
          </div>
          <div class="jjsnc-action-sheet-space"></div>
          <div class="jjsnc-action-sheet-cancel" @click="cancel">
            <span>{{_cancelTxt}}</span>
          </div>
        </div>
      </transition>
    </jjsnc-popup>
  </transition>
</template>

<script>
import jjsncPopup from "../popup/popup.vue";
import visibilityMixin from "../../common/mixins/visibility";
import popupMixin from "../../common/mixins/popup";
import localeMixin from "../../common/mixins/locale";

const COMPONENT_NAME = "jjsnc-action-sheet";
const EVENT_SELECT = "select";
const EVENT_CANCEL = "cancel";
export default {
  name: COMPONENT_NAME,
  mixins: [visibilityMixin, popupMixin, localeMixin],
  props: {
    data: {
      type: Array,
      default() {
        return [];
      }
    },
    active: {
      type: Number,
      default: -1
    },
    title: {
      type: String,
      default: ""
    },
    pickerStyle: {
      type: Boolean,
      default: false
    },
    maskClosable: {
      type: Boolean,
      default: true
    },
    cancelTxt: {
      type: String,
      default: ""
    }
  },
  computed: {
    _cancelTxt() {
      return this.cancelTxt || this.$t("cancel");
    }
  },
  methods: {
    maskClick() {
      this.maskClosable && this.cancel();
    },
    cancel() {
      this.hide();
      this.$emit(EVENT_CANCEL);
    },
    itemClick(item, index) {
      this.hide();
      this.$emit(EVENT_SELECT, item, index);
    }
  },
  components: {
    jjsncPopup
  }
};
</script>

<style lang="scss">
@import "../../common/scss/mixin.scss";
@import "../../common/scss/variable.scss";

.jjsnc-action-sheet-fade-enter,
.jjsnc-action-sheet-fade-leave-active {
  opacity: 0;
}
.jjsnc-action-sheet-fade-enter-active,
.jjsnc-action-sheet-fade-leave-active {
  transition: all 0.3s ease-in-out;
}

.jjsnc-action-sheet-panel {
  text-align: center;
  font-size: $fontsize-medium;
  background-color: $action-sheet-bgc;
}
.jjsnc-action-sheet-move-enter,
.jjsnc-action-sheet-move-leave-active {
  transform: translate3d(0, 100%, 0);
}
.jjsnc-action-sheet-move-enter-active,
.jjsnc-action-sheet-move-leave-active {
  transition: all 0.3s ease-in-out;
}
.jjsnc-action-sheet-cancel {
  background-color: $action-sheet-bgc;
}
.jjsnc-action-sheet-cancel span,
.jjsnc-action-sheet-title,
.jjsnc-action-sheet-item {
  display: block;
  padding: 17px 16px;
  margin: 0;
  text-align: center;
  overflow: hidden;
  white-space: nowrap;
  font-size: $fontsize-large;
  font-weight: normal;
  line-height: 1;
  color: $action-sheet-color;
  background-color: $action-sheet-bgc;
}

.jjsnc-action-sheet-cancel span,
.jjsnc-action-sheet-item {
  &:active {
    background-color: $action-sheet-active-bgc;
  }
}

.jjsnc-action-sheet-title {
  padding-top: 16px;
  padding-bottom: 16px;
  color: $action-sheet-title-color;
  font-size: $fontsize-large-x;
}

.jjsnc-action-sheet-content {
  overflow: hidden;
  background: $action-sheet-bgc;
}

.jjsnc-action-sheet-list {
  list-style: none;
}

.jjsnc-action-sheet-item {
  list-style: none;
  user-select: none;
  &:last-of-type {
    @include border-none();
  }
  &[data-align="left"] {
    text-align: left;
  }
  &[data-align="right"] {
    text-align: right;
  }
}

.jjsnc-action-sheet-space {
  height: 6px;
  background-color: $action-sheet-space-bgc;
}

.jjsnc-action-sheet-item_active {
  color: $action-sheet-active-color;
}

.jjsnc-action-sheet_picker {
  .jjsnc-action-sheet-space {
    height: 0;
  }
  .jjsnc-action-sheet-title {
    height: 1em;
    padding-top: 21px;
    padding-bottom: 21px;
  }

  .jjsnc-action-sheet-cancel {
    position: absolute;
    top: 0;
    background-color: transparent;
    span {
      padding-top: 23px;
      padding-bottom: 23px;
      color: $action-sheet-picker-cancel-color;
      font-size: $fontsize-medium;
      background-color: transparent;
      &:active {
        color: $action-sheet-picker-cancel-active-color;
        background-color: transparent;
      }
    }
  }
}
</style>

